...
#if defined(NDEBUG)
# error "NETAVO Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    } else if (nPrevHeight <= 500 && nPrevHeight >= 2) {
      nSubsidy = 2 * COIN;
    } else if (nPrevHeight <= 1500 && nPrevHeight >= 501) {
      nSubsidy = 3 * COIN;
    } else if (nPrevHeight <= 2000 && nPrevHeight >= 1501) {
      nSubsidy = 8 * COIN;
    } else if (nPrevHeight <= 5000 && nPrevHeight >= 2001) {
      nSubsidy = 19 * COIN;
    } else if (nPrevHeight <= 6000 && nPrevHeight >= 5001) {
      nSubsidy = 10 * COIN;
    } else if (nPrevHeight <= 10000 && nPrevHeight >= 6001) {
      nSubsidy = 6 * COIN;
    } else if (nPrevHeight <= 15000 && nPrevHeight >= 10001) {
      nSubsidy = 2 * COIN;
    } else if (nPrevHeight <= 20000 && nPrevHeight >= 15001) {
      nSubsidy = 8 * COIN;
    } else if (nPrevHeight <= 30000 && nPrevHeight >= 20001) {
      nSubsidy = 3 * COIN;
    } else if (nPrevHeight <= 50000 && nPrevHeight >= 30001) {
      nSubsidy = 9 * COIN;
    } else if (nPrevHeight <= 85000 && nPrevHeight >= 50001) {
      nSubsidy = 12 * COIN;
    } else if (nPrevHeight <= 100000 && nPrevHeight >= 85001) {
      nSubsidy = 8 * COIN;
    } else if (nPrevHeight <= 110000 && nPrevHeight >= 100001) {
      nSubsidy = 6 * COIN;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // NETAVO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(NETAVO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(NETAVO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // NETAVO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(NETAVO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(NETAVO): spork is off, skipping transaction locking checks\n");
    }

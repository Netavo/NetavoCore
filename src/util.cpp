...
const char * const BITCOIN_CONF_FILENAME = "netavo.conf";
const char * const BITCOIN_PID_FILENAME = "netavod.pid";
 ...
 * by well-meaning netavo adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "netavo" is a composite category enabling all NETAVO-related debug output
            if(ptrCategory->count(string("netavo"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NETAVOCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NETAVOCore
    // Mac: ~/Library/Application Support/NETAVOCore
    // Unix: ~/.netavocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NETAVOCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NETAVOCore";
#else
    // Unix
    return pathRet / ".netavocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty netavo.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override netavo.conf

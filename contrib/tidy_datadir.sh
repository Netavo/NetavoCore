...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete NETAVO database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no NETAVO datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old NETAVO datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected NETAVO 0.7 datadir."
    ;;
  3)
    echo "Detected NETAVO pre-0.8 datadir."
    ;;
  4)
    echo "Detected NETAVO 0.8 datadir."
    ;;
esac
